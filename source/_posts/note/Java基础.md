...
| 字符     | A，0，我     |       ‘A’，‘0’，‘我’       | 程序中必须使用[单引号]{.red}，有且仅能一个字符 |
| 字符串   | 黑马程序员   | “HelloWorld”，“黑马程序员” | 程序中必须使用[双引号]{.red}，内容可有可无     |
...
> :warning: **注意事项：**
>
> - 表达式的最终结果类型由表达式中的[最高类型]{.red}决定
> - 在表达式中，[byte、short、char]{.red}是直接转换成 [int]{.red} 类型参与运算
...
> :warning: ​**注意事项：**
>
> - 强制类型转换[**可能**造成数据（丢失）溢出]{.red}
> - 浮点型强转成整型，[直接丢掉小数部分，保留整数部分返回]{.red}
...
> - 如果单独使用放前放后是没有区别的
>
> - 非单独使用（[如在表达式中、或者同时有其它操作]{.red}）
>
>   - 在变量[前]{.red}，**先**进行变量**自增/自减**，**再使用变量**
>
>   - 在变量[后]{.red}，**先使用**变量，**再**进行变量**自增/自减**
...
> :warning: 注：[扩展的赋值运算符隐含了强制类型转换]{.red}
...
| &&   | 短路与   | 2 > 10 && 3 > 2   | 判断结果与“&”一样，过程不同：右边则不执行[左边为 false]{.red}，右边则不执行 |
| \|\| | 短路或   | 2 > 1 \| \| 3 < 5 |      判断结果与“\|”一样，过程不同：[左边为 true]{.red}， 右边则不执行       |
